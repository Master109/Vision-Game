...
namespace VisionGame
{
	...
	public class SaveAndLoadManager : SingletonMonoBehaviour<SaveAndLoadManager>
	{
		...
		public virtual void DeleteAll ()
		{
			...
			if (!usePlayerPrefs)
			{
				for (int i = AccountManager.lastUsedAccountIndex; i < AccountManager.Accounts.Length; i ++)
					Delete (i);
			}
		...
		public virtual void LoadMostRecent ()
		{
			...
			// else
				Load (AccountManager.lastUsedAccountIndex);
		}
		...
		public class SaveEntry
		{
			...
			{
				print("Save " + accountIndex + " " + saveEntryIndex);
			...
			{
				print("Load " + accountIndex + " " + saveEntryIndex);
				object value;
				foreach (MemberEntry memberEntry in memberEntries)
				{
					if (!memberEntry.isField)
